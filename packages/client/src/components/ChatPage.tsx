...
const ChatPage = ({ codebaseName, onBackToUpload }: ChatPageProps) => {
  ...
  const simulateResponse = async (userMessage: string) => {
    ...
    setTimeout(() => {
      const responses = [
        {
          content: "I found several authentication-related files in your codebase. The main auth flow appears to be implemented in `src/auth/AuthProvider.tsx` using React Context. The system supports JWT tokens with refresh token rotation.",
          thoughtProcess: thoughtSteps[thoughtSteps.length - 1]
        },
        {
          content: "Based on your database schema, I can see you're using a PostgreSQL database with Prisma ORM. The main entities are User, Post, and Comment with proper foreign key relationships. Here's what I found:",
          thoughtProcess: thoughtSteps[thoughtSteps.length - 1]
        }
      ];
      ...
      setSessions(prevSessions => prevSessions.map(session => {
        if (session.id === currentSessionId) {
          ...
          return {
            ...
            messages: updatedMessages,
            lastMessage: new Date(),
            title: session.messages.length === 1 ? userMessage.slice(0, 30) + '...' : session.title
          };
        }
        return session;
      }));
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">{codebaseName}</p>
        ...
        {currentSession ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              {currentSession.messages.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 max-w-md mx-auto">
                    I've analyzed your codebase and I'm ready to answer questions about architecture, functions, bugs, or anything else.
                  </p>
                </div>
              ) : (
                currentSession.messages.map((message) => (
                  <div key={message.id} className="space-y-2">
                    <div className={cn(
                      ...
                      <div className={cn(
                        "max-w-3xl",
                        message.type === 'user' ? "bg-blue-600 text-white rounded-2xl px-4 py-3" : ""
                      )}>
                        {message.type === 'user' ? (
                          <p>{message.content}</p>
                        ) : (
                          <Card className="p-4 shadow-sm">
                            {message.isThinking ? (
                              <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-sm text-blue-600 font-medium">Analyzing codebase...</span>
                              </div>
                            ) : message.content && (
              ...
              {isTyping && !currentSession.messages.some(m => m.isThinking) && (
                <div className="flex gap-3">
                  ...
                  <Card className="p-4 shadow-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-500">Analyzing your codebase...</span>
                    </div>
                  </Card>
                </div>
              )}
