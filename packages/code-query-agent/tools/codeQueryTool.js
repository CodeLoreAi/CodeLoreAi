...
export const codeQueryTool = tool(
  async ({ owner, repo, query, limit = 10, minRelevanceScore = 0.5 }) => {
    console.log(
      `🔍 [CODE QUERY TOOL] Searching in ${owner}/${repo} for: "${query}" (limit: ${limit})`
    );
    ...
    try {
      ...
      console.log(
        `✅ [CODE QUERY TOOL] Found ${processedResults.length}/${data.documents.length} relevant matches in ${owner}/${repo}`
      );
      console.log(
        `📊 [CODE QUERY TOOL] Average relevance: ${stats.averageRelevance}, Max: ${stats.maxRelevance}`
      );

      return JSON.stringify(result, null, 2);
    } catch (error) {
      console.error(
        `🚨 [CODE QUERY TOOL] Error querying ${owner}/${repo}:`,
        error
      );
  ...
  },
  {
    name: "query_code",
    description:
      "Search for code snippets in a GitHub repository using vector similarity with advanced filtering and ranking",
    schema: z.object({
      ...
      minRelevanceScore: z
        ...
        .optional()
        .default(0.5)
        .describe("Minimum relevance score to include results (0-2)"),
    }),
  }
);

// Helper function for batch queries
export const batchCodeQuery = tool(
  async ({ owner, repo, queries }) => {
    console.log(
      `🔍 [BATCH CODE QUERY] Running ${queries.length} queries on ${owner}/${repo}`
    );
    ...
    for (const query of queries) {
      try {
        const result = await codeQueryTool.invoke({
          owner,
          repo,
          query: query.query,
          limit: query.limit || 5,
  ...
  },
  {
    ...
    description:
      "Run multiple code queries in batch for comprehensive code analysis",
    schema: z.object({
      owner: z.string().describe("GitHub repository owner/organization name"),
      repo: z.string().describe("GitHub repository name"),
      queries: z
