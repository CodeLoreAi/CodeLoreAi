...
export const analyzeCodeTool = tool(
  async ({ owner, repo, query, analysisType }) => {
    console.log(
      `🔬 [ANALYSIS TOOL] Analyzing code in ${owner}/${repo} for: ${query} (${analysisType})`
    );
    ...
    try {
      ...
      if (data && data.documents && data.documents.length > 0) {
        const analysis = {
          ...
          insights: {
            ...
            averageRelevance: data.distances
              ? (
                  data.distances.reduce((a, b) => a + (2 - b), 0) /
                  data.distances.length
                ).toFixed(3)
              : "N/A",
            functionTypes: data.metadatas
              ? [...new Set(data.metadatas.map((m) => m.type))].join(", ")
              : "N/A",
        ...
        console.log(
          `✅ [ANALYSIS TOOL] Analysis complete for ${owner}/${repo}`
        );
  ...
  },
  {
    ...
    schema: z.object({
      ...
      analysisType: z
        .enum(["pattern", "function", "implementation", "usage"])
